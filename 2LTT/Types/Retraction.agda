...
module 2LTT.Types.Retraction where
...
retract-of-singleton (r , s , η) (c , p) = (r c) , q
 where
  q = λ y → (r c ≡⟨ ap r ((p (s y)) ⁻¹) ⟩
            (r (s y) ≡⟨ η y ⟩
            (y ▮ ))) ⁻¹
...
Σ-retract {i} {j} {k} {A} {B} {C} p = NatΣ r , NatΣ s , η'
 where
  ...
  η' : (σ : Σ A B) → Id (NatΣ r (NatΣ s σ)) σ
  η' (a , b) = (a , r a (s a b)) ≡⟨ dep-pair⁼ _ _ (refl , (η a b)) ⟩
               ( (a , b) ▮ )
...
Σ-reindexing-retract {i} {j} {k} {A} {B} {C} r (s , η) = γ , ϕ , γϕ
 where
  ...
  γϕ (x , a) = p
   where
    p : Id (r (s x) , (tr C ((η x) ⁻¹) a)) (x , a)
    p = dep-pair⁼ _ _ (η x , tr-is-retraction C (η x) a)
