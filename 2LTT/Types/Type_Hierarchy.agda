...
is-contr A = Σ A (λ a → ((x : A) →  Id x a))

center : {i : Level} (A : UU i) → is-contr A → A
center A (a , p) = a

centrality : {i : Level} (A : UU i) (P : is-contr A) (x : A) → Id x (center A P)
...
is-type (succ-𝕋 t) A = (a' : A) → (a : A) → is-type t (Id a' a)
...
is-set-is-contr A = (is-set-is-prop A) ∘ (is-prop-is-contr A)
...
is-prop-contr {i} A = is-prop-all-elements-equal λ {(a , p) → λ {(a' , p')
                                                 → dep-pair⁼ {i} {i} {A} {B = λ a → (Π A (λ x → Id x a))} (a , p) (a' , p') 
                                                   (p' a ,  
                                                   all-elements-equal-is-prop
                                                    (Π-type-prop λ a'' → (is-set-is-contr A (a , p)) a'' a')
