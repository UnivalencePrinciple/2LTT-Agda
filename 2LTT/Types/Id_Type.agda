...
=ₛ-to-Id : {i : Level} {A : UU i} {a b : A} → a =ₛ b → Id a b
=ₛ-to-Id reflₛ = refl
...
ap-comp :  {i j k : Level} {A : UU i} {B : UU j} {C : UU k}
           (g : B → C) (f : A → B) {x y z : A} (p : Id {i} x y)
...
tr-is-retraction : {i j : Level} {A : UU i} (P : A → UU j) {a a' : A} (p : Id a a') → (z : P a') → Id (tr P p (tr P (p ⁻¹) z)) z  
tr-is-retraction P refl z = refl

tr-is-section : {i j : Level} {A : UU i} (P : A → UU j) {a a' : A} (p : Id a a') → (z : P a) → Id (tr P (p ⁻¹) (tr P p z)) z  
tr-is-section P refl z = refl

tr-cong : {i j : Level} {A : UU i} {P : A → UU j} {x y : A} {p q : Id x y} {b : P x}
...
tr-comp-func : {i j k : Level} {A : UU i} {B : UU j} {P : B → UU k}
               {f : A → B} {x y : A} (p : Id {i} x y) (u : P (f x))
               → Id {k} (tr {i} {k} (λ a → P (f a)) p u) (tr {j} {k} P (ap {i} {j} f p) u)
tr-comp-func refl u = refl
...
tr-fam-ap : {i j : Level} {A : UU i} {P : A → UU j} {x y : A} {p : Id x y} {f g : P x → P x}{z : P x}
...
htpy-pair⁼-inv-pair⁼ : {i j : Level} {A : UU i} {B : UU j} (x y : A × B)
...
dep-pair-Id : {i j : Level} {A : UU i} {B : A → UU j} (w w' : Σ A B)
              → UU (i ⊔ j)
dep-pair-Id w w' = Σ (Id (pr1 w) (pr1 w')) (λ p → Id (tr _ p (pr2 w)) (pr2 w'))
...
htpy-dep-pair⁼-inv-dep-pair⁼ : {i j : Level} {A : UU i} {B : A → UU j} (x y : Σ A B)
...
postulate
 FUNEXT : {i j : Level} {A : UU i} {B : A → UU j} {f g : (x : A) → B x}
          → (Π (f ~ g) (λ H → let T = (Σ (Id f g) (λ p → Id (happly p) H)) in Σ T (λ a → ((x : T) →  Id x a)))) 
