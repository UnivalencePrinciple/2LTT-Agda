...
module 2LTT_C.Types.Id_Type where
...
=ᵉ-to-Id : {i : Level} {A : UU i} {a b : A} → (c a) =ᵉ (c b) → Id a b
=ᵉ-to-Id reflᵉ = refl

--identity obtained by coerced equality
cic-eq : {i : Level} {A : UU i} {x y : C A} → x =ᵉ y → Id (ic x) (ic y)
cic-eq reflᵉ = refl
